...
`tool_registry.json`은 도구별 최신 스냅샷과 change history를 누적 저장한다.
즉, `first_seen_at/last_checked_at`, 상태 변화, taxonomy/score/review 변화 추적에 사용할 수 있다.
...
- 결과의 `management.lifecycle_state`는 현재 `final_status`를 기준으로 기록하며, 운영 확장을 위해 `discovered/screened/archived` 값도 예약돼 있다.
...
- 테스트는 추가됐지만 통합/회귀 범위가 아직 제한적이다.
- DB 적재 코드가 없다.
- 출력 채널이 로컬 JSON(`results + tool_registry`) 중심이다.
